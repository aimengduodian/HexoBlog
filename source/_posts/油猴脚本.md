...
---
## 是什么
Tampermonkey中文名俗称油猴，是一款浏览器上的扩展，用户可以通过Tampermonkey添加和使用脚本，而脚本是一种可以修改网页JavaScript的程序。

Tampermonkey最早由扬·比诺克（Jan Biniok）于2010年5月创建。它最初只是Greasemonkey的一个脚本，之后作为一款独立的扩展在Google Chrome浏览器上推出。2019年，Tampermonkey的用户已经超过1000万。Tampermonkey是Chrome网上应用店上用户数量超过1000万的13个扩展之一。

## 如何用
火狐浏览器和edge浏览器。自Windows抛弃ie浏览器后，edge浏览器的内核用的是chromium，所以chrome浏览器的插件，大部分在edge中都能使用。Tampermonkey插件在浏览器的应用商店下载安装后就可以开心的写代码了。

### 脚本引入
如果我们编写的脚本需要使用的jQuery语法，可以在脚本上引入  
...
### 应用
网页限制解除脚本，by Cat73 & iqxin(修改)。慢慢研究学习。
